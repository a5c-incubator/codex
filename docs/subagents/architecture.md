...
1. **Discovery** — `ManifestLoader` polls/watches project, CLI JSON, user, and plugin scopes in priority order, tagging every manifest with its provenance and digest.
...
1. **Manifest watcher ergonomics** — Registry-side plumbing now lives in `AgentRegistryWatch` (debounced + telemetry); remaining work is wiring the CLI/TUI surfaces to opt into the shared handle.
