//! Error types raised while parsing and validating subagent manifests.
...
pub enum ManifestError {
    ...
    WatchUnsupported,
    /// Failed to configure a file system watcher for a discovery target.
    #[error("failed to watch {path}: {source}")]
    WatchSetup {
        /// Path that failed to be observed.
        path: PathBuf,
