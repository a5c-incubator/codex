...
fn loader_snapshot() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let manifests = loader.load(&sample_targets())?;
    assert_json_snapshot!("loader_manifests", manifests);
    Ok(())
}
...
fn scope_label(scope: &DiscoveryScope) -> String {
    match scope {
        ...
        DiscoveryScope::Plugin { plugin, .. } => format!("plugin: {}", plugin.0),
    }
}
